...
int main(int argc, char *argv[]) {
    union data key, encrypted, decrypted;
    decrypted.bytes = 0x0123456789ABCDEF;
    key.bytes = 0x133457799BBCDFF1;

    printf("Before: %s, as bytes: %lx\n", decrypted.word, decrypted.bytes);

    encrypted.bytes = encrypt(decrypted.bytes, key.bytes);
    printf("Encrypted: %s, as hex: %lx\n", encrypted.word, encrypted.bytes);

    decrypted.bytes = decrypt(encrypted.bytes, key.bytes);
    printf("Decrypted: %s, as bytes: %lx\n", decrypted.word, decrypted.bytes);
